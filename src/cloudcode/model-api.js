...
export async function getSubscriptionTier(token) {
  ...
  for (const endpoint of LOAD_CODE_ASSIST_ENDPOINTS) {
    try {
      ...
      if (!response.ok) {
        logger.warn(
          `[CloudCode] loadCodeAssist error at ${endpoint}: ${response.status}`,
        );
        continue;
      }
  ...
  modelCache.fetchPromise = (async () => {
    try {
      ...
      if (data && data.models) {
        ...
        modelCache.lastFetched = Date.now();
        logger.debug(
          `[CloudCode] Model cache populated with ${validIds.length} models`,
        );
      }
    } catch (error) {
      logger.warn(
        `[CloudCode] Failed to populate model cache: ${error.message}`,
      );
