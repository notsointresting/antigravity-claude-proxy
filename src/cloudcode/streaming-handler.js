  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (availableAccounts.length === 0) {
      ...
      if (accountManager.isAllAccountsInvalid()) {
        ...
        const reasons = [
          ...new Set(
            invalidAccounts.map((a) => a.invalidReason).filter(Boolean),
          ),
        ];
        throw new Error(
      ...
      if (
        ...
        accountManager.isAllRateLimited(model)
      ) {
    ...
    if (account && waitMs > 0) {
      logger.debug(
        `[CloudCode] Throttling request (${waitMs}ms) - fallback mode active`,
      );
      await sleep(waitMs);
    }
    ...
    if (!account) {
      logger.warn(
        `[CloudCode] Strategy returned no account for ${model} (attempt ${attempt + 1}/${maxAttempts})`,
      );
      continue;
    }
    ...
    try {
      ...
      let endpointIndex = 0;
      const endpoints = getPreferredEndpoints();
          ...
          if (!response.ok) {
            ...
            if (response.status === 429) {
              ...
              const consecutiveFailures =
                accountManager.getConsecutiveFailures?.(account.email) || 0;
              ...
              if (isModelCapacityExhausted(errorText)) {
                if (capacityRetryCount < MAX_CAPACITY_RETRIES) {
                  ...
                  const waitMs =
                    resetMs || CAPACITY_BACKOFF_TIERS_MS[tierIndex];
                  capacityRetryCount++;
              ...
              if (resetMs !== null && resetMs < 1000) {
                ...
                logger.info(
                  `[CloudCode] Short rate limit on ${account.email} (${resetMs}ms), waiting and retrying...`,
                );
                await sleep(waitMs);
                // Don't increment endpointIndex - retry same endpoint
                continue;
              ...
              if (backoff.isDuplicate) {
                const smartBackoffMs = calculateSmartBackoff(
                  errorText,
                  resetMs,
                  consecutiveFailures,
                );
                logger.info(
                  `[CloudCode] Skipping retry due to recent rate limit on ${account.email} (attempt ${backoff.attempt}), switching account...`,
                );
              ...
              if (
                backoff.attempt === 1 &&
                smartBackoffMs <= DEFAULT_COOLDOWN_MS
              ) {
                ...
                logger.info(
                  `[CloudCode] First rate limit on ${account.email}, quick retry after ${formatDuration(waitMs)}...`,
                );
                await sleep(waitMs);
                // Don't increment endpointIndex - retry same endpoint
                continue;
                ...
                logger.info(
                  `[CloudCode] Quota exhausted for ${account.email} (${formatDuration(smartBackoffMs)}), switching account after ${formatDuration(SWITCH_ACCOUNT_DELAY_MS)} delay...`,
                );
                ...
                accountManager.markRateLimited(account.email, waitMs, model);
                logger.info(
                  `[CloudCode] Rate limit on ${account.email} (attempt ${backoff.attempt}), waiting ${formatDuration(waitMs)}...`,
                );
                await sleep(waitMs);
                // Don't increment endpointIndex - retry same endpoint
                continue;
            ...
            if (
              ...
              if (capacityRetryCount < MAX_CAPACITY_RETRIES) {
                ...
                accountManager.incrementConsecutiveFailures(account.email);
                logger.info(
                  `[CloudCode] ${response.status} Model capacity exhausted, retry ${capacityRetryCount}/${MAX_CAPACITY_RETRIES} after ${formatDuration(waitMs)}...`,
                );
                await sleep(waitMs);
                // Don't increment endpointIndex - retry same endpoint
                continue;
              ...
              logger.warn(
                `[CloudCode] Max capacity retries (${MAX_CAPACITY_RETRIES}) exceeded on ${response.status}, switching account`,
              );
              accountManager.markRateLimited(
                account.email,
                BACKOFF_BY_ERROR_TYPE.MODEL_CAPACITY_EXHAUSTED,
                model,
              );
            ...
            if (response.status === 400) {
              logger.error(
                `[CloudCode] Invalid request (400): ${errorText.substring(0, 200)}`,
              );
            ...
            if (response.status === 403 && isValidationRequired(errorText)) {
              ...
              logger.warn(
                `[CloudCode] 403 VALIDATION_REQUIRED/PERMISSION_DENIED for ${account.email}, marking invalid and rotating account...`,
              );
              accountManager.markInvalid(
                account.email,
      ...
      if (isAccountForbiddenError(error)) {
        ...
        logger.warn(
          `[CloudCode] Account ${account.email} forbidden (403 VALIDATION_REQUIRED), trying next...`,
        );
        continue;
      }
      // Handle 5xx errors
      if (
        ...
        const currentFailures = accountManager.getConsecutiveFailures(
          account.email,
        );
        if (currentFailures + 1 >= MAX_CONSECUTIVE_FAILURES) {
          logger.warn(
            `[CloudCode] Account ${account.email} has ${currentFailures + 1} consecutive failures, applying extended cooldown (${formatDuration(EXTENDED_COOLDOWN_MS)})`,
          );
          accountManager.markRateLimited(
            account.email,
            EXTENDED_COOLDOWN_MS,
          ...
          accountManager.incrementConsecutiveFailures(account.email);
          logger.warn(
            `[CloudCode] Account ${account.email} failed with 5xx stream error (${currentFailures + 1}/${MAX_CONSECUTIVE_FAILURES}), trying next...`,
          );
        }
        continue;
      }
      ...
      if (isNetworkError(error)) {
        ...
        if (currentFailures + 1 >= MAX_CONSECUTIVE_FAILURES) {
          logger.warn(
            `[CloudCode] Account ${account.email} has ${currentFailures + 1} consecutive network failures, applying extended cooldown (${formatDuration(EXTENDED_COOLDOWN_MS)})`,
          );
          accountManager.markRateLimited(
            account.email,
            EXTENDED_COOLDOWN_MS,
          ...
          accountManager.incrementConsecutiveFailures(account.email);
          logger.warn(
            `[CloudCode] Network error for ${account.email} (stream) (${currentFailures + 1}/${MAX_CONSECUTIVE_FAILURES}), trying next account... (${error.message})`,
          );
        }
        await sleep(1000);
        continue;
