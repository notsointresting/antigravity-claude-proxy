...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // Bolt Optimization: Replaced multiple .map().filter() chains with single for loops
        // to prevent large intermediate array allocations and reduce GC overhead during account selection
        const candidates = [];
