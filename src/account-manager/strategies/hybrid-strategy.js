...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // Last resort: usable but might fail health or token checks
            lastResort.push({ account, index });

            // Emergency: usable and has tokens, but might fail health check
            if (hasTokens) {
                emergency.push({ account, index });

                // Normal/Quota fallback: usable, has tokens, and is healthy
                if (isHealthy) {
