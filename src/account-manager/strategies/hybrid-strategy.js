...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // Performance optimization: Use a single pass direct loop instead of multiple
        // chained Array.map().filter() calls to prevent unnecessary allocations
        // and reduce garbage collection pressure on the hot path.
        const normal = [];
        const fallback = [];
        const emergency = [];
        const lastResort = [];
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            const cand = { account, index };
            lastResort.push(cand);
