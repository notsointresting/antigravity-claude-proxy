...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        ...
        // Optimize: Single pass through accounts instead of multiple map/filter chains
        // This avoids intermediate array allocations and redundant checks on the hot path
        for (let i = 0; i < accounts.length; i++) {
            const account = accounts[i];
            const candidate = { account, index: i };

            // Check basic usability (not rate-limited, not disabled)
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }

            // All usable accounts are eligible for last resort
            lastResort.push(candidate);

            // Check token availability (needed for emergency and above)
            const hasTokens = this.#tokenBucketTracker.hasTokens(account.email);
            if (!hasTokens) {
                continue;
            }

            // Accounts with tokens are eligible for emergency fallback
            emergency.push(candidate);

            // Check health (needed for quota and normal)
            const isHealthy = this.#healthTracker.isUsable(account.email);
            if (!isHealthy) {
                continue;
            }

            // Accounts with health & tokens are eligible for quota fallback
            quota.push(candidate);

            // Check quota limits (needed for normal)
            const effectiveThreshold = account.modelQuotaThresholds?.[modelId]
                ?? account.quotaThreshold
                ?? (config.globalQuotaThreshold || undefined);

            const isCritical = this.#quotaTracker.isQuotaCritical(account, modelId, effectiveThreshold);
            if (isCritical) {
            ...
            // Accounts that pass all checks are normal candidates
