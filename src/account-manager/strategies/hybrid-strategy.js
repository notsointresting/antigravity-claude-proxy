...
export class HybridStrategy extends BaseStrategy {
    ...
    selectAccount(accounts, modelId, options = {}) {
        ...
        // Score candidates and track the best one in a single pass to avoid creating/sorting a new array
        let best = null;
    ...
    #getCandidates(accounts, modelId) {
        // Optimize: use pre-allocated arrays and a single-pass loop to avoid intermediate allocations
        // from chaining .map().filter() and wrapping objects like { account, index }.
        const candidates = [];
        const fallback = [];
        const emergency = [];
        const lastResort = [];
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // Basic usability check (required for all fallback levels)
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }

            // Record as last resort since it passed basic usability
            ...
            // Record as emergency since it has tokens
            ...
            // Record as fallback since it's usable, has tokens, and is healthy
            fallback.push({ account, index });

            // Quota availability check (exclude critically low quota for normal candidates)
            ...
            // Passed all checks, it's a normal candidate
