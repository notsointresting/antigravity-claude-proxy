...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // Optimize hot path by evaluating all fallback conditions in a single pass
        // This avoids multiple .map().filter() allocations and redundant iterations
        const candidates = [];
        const fallback = [];
        const emergency = [];
        const lastResort = [];
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // Base check for all tiers
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }

            // lastResort tier (needs nothing else)
            lastResort.push(item);

            // emergency tier (needs tokens)
            ...
            // fallback tier (needs tokens + health)
            ...
            // normal tier (needs tokens + health + quota)
