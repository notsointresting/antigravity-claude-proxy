...
export class HybridStrategy extends BaseStrategy {
    ...
    selectAccount(accounts, modelId, options = {}) {
        ...
        // Bolt: Optimize with pre-allocated loop instead of .map() to avoid array resize overhead
        ...
        for (let i = 0; i < candidates.length; i++) {
            ...
            scored[i] = {
                ...
                score: this.#calculateScore(candidate.account, modelId)
            };
        }
    ...
    #getCandidates(accounts, modelId) {
        // Bolt: Optimize hot path by evaluating all fallback conditions simultaneously
        // in a single pass instead of creating up to 4 intermediate arrays with chained
        // .map().filter() calls. This drastically reduces object allocations.
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // Basic usability check (required for all levels)
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }

            // Create single object reference to avoid redundant allocations across arrays
            const candidate = { account, index };

            // At this point it's valid for lastResort (skips health and token checks)
            lastResortCandidates.push(candidate);

            // Token availability check (required for normal, quota, emergency)
            if (!this.#tokenBucketTracker.hasTokens(account.email)) {
                continue;
            }

            // At this point it's valid for emergency (skips health check)
            emergencyCandidates.push(candidate);

            // Health score check (required for normal, quota)
            if (!this.#healthTracker.isUsable(account.email)) {
                continue;
            }

            // At this point it's valid for quota fallback
            quotaCandidates.push(candidate);

            // Quota availability check (only normal level requires non-critical quota)
            ...
            // Valid for normal use!
            normalCandidates.push(candidate);
        }

        // Return the best available candidate pool in order of preference
        ...
        if (quotaCandidates.length > 0) {
            ...
            return { candidates: quotaCandidates, fallbackLevel: 'quota' };
        }
