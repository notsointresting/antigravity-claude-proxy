...
export class HybridStrategy extends BaseStrategy {
    ...
    selectAccount(accounts, modelId, options = {}) {
        ...
        // Score candidates and find the best in a single pass to avoid map() and sort() overhead
        let best = null;
    ...
    #getCandidates(accounts, modelId) {
        ...
        // Categorize accounts in a single pass to eliminate multiple map().filter() allocations
        for (let index = 0; index < accounts.length; index++) {
            const account = accounts[index];

            // Basic usability check (not disabled, not rate-limited for this model)
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }

            // lastResort: Bypass health and token checks entirely
            ...
            if (!this.#healthTracker.isUsable(account.email)) {
                // emergency: Usable and has tokens, but unhealthy
            ...
            if (this.#quotaTracker.isQuotaCritical(account, modelId, effectiveThreshold)) {
                ...
                quotaFallback.push({ account, index });
            } else {
                normal.push({ account, index });
            }
        }
        ...
        // If no candidates after quota filter, fall back to all usable accounts with critical quota
