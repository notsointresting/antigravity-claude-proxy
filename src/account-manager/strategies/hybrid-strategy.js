...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // 1. Basic usability check (not rate-limited, not disabled)
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }

            const item = { account, index };
            // Bypass BOTH health AND token bucket checks
            ...
            // Valid candidate but ignoring quota
            fallback.push(item);

            // 4. Quota availability check (exclude critically low quota)
