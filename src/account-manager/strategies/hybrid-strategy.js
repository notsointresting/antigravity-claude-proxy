...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // ⚡ Bolt: Use a single-pass loop instead of multiple chained .map().filter()
        // to avoid allocating `{ account, index }` wrapper objects and multiple iterations.
        const candidates = [];
        const fallback = [];
        const emergency = [];
        const lastResort = [];
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // If usable, it qualifies for lastResort
            ...
            // If usable + has tokens, it qualifies for emergency
            ...
            // If usable + has tokens + healthy, it qualifies for fallback (ignores quota)
