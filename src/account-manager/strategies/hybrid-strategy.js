...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        ...
        // Single pass over accounts to classify them into candidate tiers
        for (let index = 0; index < accounts.length; index++) {
            const account = accounts[index];
            const candidateInfo = { account, index };

            // Base check: account must be usable (not rate-limited, not disabled)
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }

            // At this point, it qualifies for lastResort
            ...
            // At this point, it qualifies for emergency (usable + has tokens)
            ...
            // At this point, it qualifies for fallback (usable + has tokens + healthy)
