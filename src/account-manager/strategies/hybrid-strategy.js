...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // Optimize: use direct loops instead of array map/filter chain
        // This avoids creating intermediate arrays and objects, significantly improving
        // performance when called frequently under high load.
        const candidates = [];
        const fallback = [];
        const emergency = [];
        const lastResort = [];
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // Token availability check (required for emergency, fallback, normal)
            if (!this.#tokenBucketTracker.hasTokens(account.email)) continue;

            emergency.push(entry);

            // Health score check (required for fallback, normal)
            if (!this.#healthTracker.isUsable(account.email)) continue;

            fallback.push(entry);

            // Quota availability check (required for normal level only)
