...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // Use a single-pass loop to avoid multiple array allocations and GC overhead
        const candidates = [];
        const fallback = [];
        const emergency = [];
        const lastResort = [];
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // Basic usability check (not rate-limited, not disabled)
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }

            // If usable, it's at least a last resort candidate
            ...
            // If it has tokens, it's at least an emergency candidate
            ...
            // If it's healthy, it's at least a fallback candidate
            ...
            // If it passes all checks, it's a normal candidate
