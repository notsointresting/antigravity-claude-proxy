...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        const candidates = [];
        // Bolt ⚡ Optimization: Use a single-pass loop instead of .map().filter()
        // to avoid allocating intermediate wrapper objects and reduce GC pressure on this hot path.
