...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // Fast path: compute candidates in single pass
        const candidates = [];
        const fallback = [];
        const emergency = [];
        const lastResort = [];
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // Last resort: bypass health and token checks
            ...
            if (hasTokens) {
                // Emergency: bypass health checks
            ...
            if (hasTokens && isHealthy) {
                // Fallback: bypass quota checks
                fallback.push({ account, index });

                // Normal candidate: check quota
