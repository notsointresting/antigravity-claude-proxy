...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // Use single-pass loops to avoid intermediate array allocations (.map().filter() chains)
        const candidates = [];
        const fallback = [];
        const emergency = [];
        const lastResort = [];
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // Last resort: usable but bypass health and token bucket checks
            lastResort.push(item);

            // Emergency: usable and has tokens (bypass health)
            const hasTokens = this.#tokenBucketTracker.hasTokens(account.email);
            if (!hasTokens) continue;
            emergency.push(item);

            // Fallback: usable, has tokens, and healthy (bypass quota)
            const isHealthy = this.#healthTracker.isUsable(account.email);
            if (!isHealthy) continue;
            fallback.push(item);

            // Normal candidate: usable, has tokens, healthy, and acceptable quota
