...
export class HybridStrategy extends BaseStrategy {
    ...
    selectAccount(accounts, modelId, options = {}) {
        ...
        // Score candidates and find the best one in a single pass (O(n))
        // This avoids .map() and .sort() (O(n log n)) array allocations
        let best = null;
    ...
    #getCandidates(accounts, modelId) {
        ...
        // Single-pass optimization to replace multiple .map().filter() chains
        // reducing array allocations and garbage collection overhead
        for (let index = 0; index < accounts.length; index++) {
            ...
            // Skip health and token bucket checks entirely for lastResort
            const hasTokens = this.#tokenBucketTracker.hasTokens(account.email);
            if (!hasTokens) continue;

            // Skip health check - use "least bad" account for emergency
