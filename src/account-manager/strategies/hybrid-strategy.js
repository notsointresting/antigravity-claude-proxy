...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            if (!hasTokens) {
                // Fails token check -> Last Resort only
            ...
            if (!isHealthy) {
                // Has tokens but unhealthy -> Emergency or Last Resort
                emergency.push(candidate);
                lastResort.push(candidate);
                continue;
            }

            // At this point: Account is Usable + Has Tokens + Healthy -> Qualifies for Fallback (Quota Bypass)
            ...
            if (this.#quotaTracker.isQuotaCritical(account, modelId, effectiveThreshold)) {
                ...
                // Passes all checks -> Normal candidate
