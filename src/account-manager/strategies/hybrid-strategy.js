...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // Optimization: Use a single-pass loop to categorize all accounts into their highest
        // passing fallback tier. This avoids multiple chained .map().filter() calls, reduces
        // wrapper object allocations, and eliminates redundant property/method evaluations.
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // Tier 4: Basic usability (Last Resort)
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }
            lastResortCandidates.push({ account, index });

            // Tier 3: Has tokens (Emergency)
            if (!this.#tokenBucketTracker.hasTokens(account.email)) {
                continue;
            }
            emergencyCandidates.push({ account, index });

            // Tier 2: Healthy account (Quota fallback)
            if (!this.#healthTracker.isUsable(account.email)) {
                continue;
            }
            quotaCandidates.push({ account, index });

            // Tier 1: Good quota (Normal)
            ...
            normalCandidates.push({ account, index });
        }
