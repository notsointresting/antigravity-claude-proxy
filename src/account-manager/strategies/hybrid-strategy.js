...
export class HybridStrategy extends BaseStrategy {
    ...
    selectAccount(accounts, modelId, options = {}) {
        ...
        // Score candidates and find the best one in O(n)
        const now = Date.now();
        let best = null;
        let maxScore = -Infinity;

        for (const candidate of candidates) {
            const score = this.#calculateScore(candidate.account, modelId, now);
            if (score > maxScore) {
                maxScore = score;
    ...
    #getCandidates(accounts, modelId) {
        ...
        // Single pass over accounts to categorize them
        for (let index = 0; index < accounts.length; index++) {
            const account = accounts[index];
            const candidateObj = { account, index };

            // 1. Basic usability check (required for ALL levels)
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }

            // At this point, it qualifies for lastResort (bypasses health and tokens)
            lastResort.push(candidateObj);
            ...
            // At this point, it qualifies for emergency (bypasses health)
            emergency.push(candidateObj);

            // 3. Health score check
            const isHealthy = this.#healthTracker.isUsable(account.email);
            if (!isHealthy) {
                continue;
            }

            // At this point, it qualifies for fallback (bypasses quota)
            fallback.push(candidateObj);

            // 4. Quota availability check (normal candidates)
     ...
     */
    #calculateScore(account, modelId, now) {
        ...
        const timeSinceLastUse = Math.min(now - lastUsed, 3600000); // Cap at 1 hour
