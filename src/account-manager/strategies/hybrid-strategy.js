...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // Evaluate multiple conditions in a single pass to avoid array allocations from .map().filter()
        const candidates = [];
        const fallback = [];
        const emergency = [];
        const lastResort = [];
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // 1. Last resort level: Basic usability check
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }
            lastResort.push(item);

            // 2. Emergency level: Basic usability + Tokens
            if (!this.#tokenBucketTracker.hasTokens(account.email)) {
                continue;
            }
            emergency.push(item);

            // 3. Fallback level: Basic usability + Tokens + Health score
            if (!this.#healthTracker.isUsable(account.email)) {
                continue;
            }
            fallback.push(item);

            // 4. Normal candidates level: Basic usability + Tokens + Health + Quota
