...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // Bolt: Optimized to use a single-pass loop instead of multiple chained Array.map().filter() operations
        // This evaluates all conditions simultaneously without intermediate array allocations.
        const candidates = [];
        const fallback = [];
        const emergency = [];
        const lastResort = [];
        ...
        for (let index = 0; index < accounts.length; index++) {
            ...
            // Base usability is required for all levels
            if (!this.isAccountUsable(account, modelId)) continue;

            const item = { account, index };
            const email = account.email;

            // Last resort: requires only basic usability
            lastResort.push(item);

            const hasTokens = this.#tokenBucketTracker.hasTokens(email);

            // Emergency: requires usability + tokens (bypasses health)
            if (hasTokens) {
                emergency.push(item);
            }

            const isHealthy = this.#healthTracker.isUsable(email);

            // Fallback: requires usability + tokens + health (bypasses quota)
            if (hasTokens && isHealthy) {
                fallback.push(item);

                // Normal: requires all conditions, including quota
                ...
                if (this.#quotaTracker.isQuotaCritical(account, modelId, effectiveThreshold)) {
                    logger.debug(`[HybridStrategy] Excluding ${email}: quota critically low for ${modelId} (threshold: ${effectiveThreshold ?? 'default'})`);
                } else {
