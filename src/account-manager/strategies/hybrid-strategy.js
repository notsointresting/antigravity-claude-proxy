...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // [Bolt: Performance Improvement] Replaced chaining `.map().filter()` with direct `for` loops
        // to avoid intermediate array allocations and reduce garbage collection pressure on a hot path.
        const candidates = [];
