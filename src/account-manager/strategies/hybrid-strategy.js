...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        // Optimize by using a single pass over the accounts array
        // instead of chained map/filter calls to reduce GC pressure
        const candidates = [];
        const fallback = [];
        const emergency = [];
        const lastResort = [];
        ...
        for (let i = 0; i < accounts.length; i++) {
            ...
            const entry = { account, index: i };

            // Account is usable, add to lastResort fallback
            lastResort.push(entry);
            ...
            // Account has tokens, add to emergency fallback
            ...
            // Account is healthy, add to quota fallback
            fallback.push(entry);
