...
export class HybridStrategy extends BaseStrategy {
    ...
    #getCandidates(accounts, modelId) {
        ...
        for (let i = 0; i < accounts.length; i++) {
            ...
            // Last resort: basic usability check (not rate-limited, not disabled)
            if (!this.isAccountUsable(account, modelId)) {
                continue;
            }

            const wrapped = { account, index: i };
            lastResort.push(wrapped);

            // Emergency: needs tokens
            if (!this.#tokenBucketTracker.hasTokens(account.email)) {
                continue;
            }

            emergency.push(wrapped);

            // Fallback: needs health check
            if (!this.#healthTracker.isUsable(account.email)) {
                continue;
            }

            fallback.push(wrapped);

            // Normal: needs quota check
